...
require_once __DIR__.'/SyncHttpClient.class.php';
require_once __DIR__.'/SyncHttpException.class.php';
require_once __DIR__.'/SyncSchemaMigrator.class.php';
require_once __DIR__.'/../lib/syncodoo_config.lib.php';

class SyncOdooLegacy
{
    public $db;
    public $odoo_url;
    public $odoo_db;
    ...
    private function odooJsonRpc($service, $method, $args)
    {
        ...
        if ($encodedPayload === false) {
            throw new Exception('Encodage JSON-RPC Odoo impossible.');
        }

        $response = $this->httpClient->request([
            'context' => 'odoo.jsonrpc '.$service.'.'.$method,
            'url' => $url,
            'method' => 'POST',
    ...
    public function __construct($db)
    {
        ...
        $this->odoo_password = syncodoo_get_secret_from_conf($conf, 'SYNCODOO_ODOO_PASSWORD', ['SYNCODOO_ODOO_PASS']);
        ...
        $this->doli_api_key = ($user && !empty($user->api_key)) ? $user->api_key : $fallbackApiKey;
        ...
        $this->httpClient = new SyncHttpClient(10, 30, 2, 250);
    }
    ...
    private function prepareSql($sql, array $params = [])
    {
        ...
        foreach ($params as $name => $definition) {
            ...
            $replacements[$placeholder] = $this->sqlValue($value, $type);
        }

        return strtr($sql, $replacements);
    }

    private function dbQueryParams($sql, array $params = [], $context = 'SQL error')
    {
        $preparedSql = $this->prepareSql($sql, $params);
        $resql = $this->db->query($preparedSql);
        if ($resql === false) {
            throw new Exception($context.': '.$this->db->lasterror());
        }

        return $resql;
    }

    private function ensureSchemaMigrated()
    {
    ...
    private function getBankSyncMappingByOdooId($odooTransactionId)
    {
        ...
        $sql = "SELECT * FROM ".MAIN_DB_PREFIX."syncodoo_bank_map WHERE odoo_transaction_id = :odoo_transaction_id LIMIT 1";
        $resql = $this->dbQueryParams($sql, [
            'odoo_transaction_id' => ['value' => $odooTransactionId, 'type' => 'int'],
        ], 'Erreur lecture mapping bancaire Odoo');
        if ($resql) {
    ...
    private function getBankSyncMappingByDolibarrLineId($bankLineId)
    {
        ...
        $sql = "SELECT * FROM ".MAIN_DB_PREFIX."syncodoo_bank_map WHERE dolibarr_bank_line_id = :bank_line_id LIMIT 1";
        $resql = $this->dbQueryParams($sql, [
            'bank_line_id' => ['value' => $bankLineId, 'type' => 'int'],
        ], 'Erreur lecture mapping bancaire Dolibarr');
        if ($resql) {
            $obj = $this->db->fetch_object($resql);
            if ($obj) {
                return [
    ...
    private function saveBankSyncMapping($odooTransactionId, $bankLineId, $bankAccountId, $odooJournalId, $direction, $odooWriteDate = '')
    {
        ...
        $this->runInTransaction(function () use ($odooTransactionId, $bankLineId, $bankAccountId, $odooJournalId, $direction, $odooWriteDate) {
            $existing = null;
            if ($odooTransactionId > 0) {
            ...
            $params = [
                'odoo_transaction_id' => ['value' => ($odooTransactionId > 0 ? $odooTransactionId : null), 'type' => ($odooTransactionId > 0 ? 'int' : 'null')],
                'dolibarr_bank_line_id' => ['value' => ($bankLineId > 0 ? $bankLineId : null), 'type' => ($bankLineId > 0 ? 'int' : 'null')],
                'dolibarr_bank_account_id' => ['value' => ($bankAccountId > 0 ? $bankAccountId : null), 'type' => ($bankAccountId > 0 ? 'int' : 'null')],
                'odoo_journal_id' => ['value' => ($odooJournalId > 0 ? $odooJournalId : null), 'type' => ($odooJournalId > 0 ? 'int' : 'null')],
                'sync_direction' => ['value' => $direction, 'type' => 'string'],
                'odoo_write_date' => ['value' => ($odooWriteDate !== '' ? $odooWriteDate : null), 'type' => ($odooWriteDate !== '' ? 'string' : 'null')],
            ];

            if ($existing) {
                $params['rowid'] = ['value' => (int) $existing['rowid'], 'type' => 'int'];
                $sql = "UPDATE ".MAIN_DB_PREFIX."syncodoo_bank_map
                        SET odoo_transaction_id = :odoo_transaction_id,
                            dolibarr_bank_line_id = :dolibarr_bank_line_id,
                            dolibarr_bank_account_id = :dolibarr_bank_account_id,
                            odoo_journal_id = :odoo_journal_id,
                            sync_direction = :sync_direction,
                            odoo_write_date = :odoo_write_date
                        WHERE rowid = :rowid";
                $this->dbQueryParams($sql, $params, 'Erreur update mapping bancaire');
                return;
            }
            ...
            $sql = "INSERT INTO ".MAIN_DB_PREFIX."syncodoo_bank_map (datec, odoo_transaction_id, dolibarr_bank_line_id, dolibarr_bank_account_id, odoo_journal_id, sync_direction, odoo_write_date)
                    VALUES (NOW(), :odoo_transaction_id, :dolibarr_bank_line_id, :dolibarr_bank_account_id, :odoo_journal_id, :sync_direction, :odoo_write_date)";
            $this->dbQueryParams($sql, $params, 'Erreur insert mapping bancaire');
        });
    }
    ...
    private function getDolibarrThirdpartyIdByOdooId($odooId)
    {
        ...
        $sql = 'SELECT fk_object FROM '.MAIN_DB_PREFIX.'societe_extrafields WHERE odoo_id = :odoo_id LIMIT 1';
        $resql = $this->dbQueryParams($sql, [
            'odoo_id' => ['value' => (int) $odooId, 'type' => 'int'],
        ], 'Erreur lecture correspondance tiers Odoo');
        if ($resql) {
            $obj = $this->db->fetch_object($resql);
            if ($obj) {
    ...
    private function updateThirdpartyMapping($dolId, $odooId)
    {
        ...
        $this->runInTransaction(function () use ($dolId, $odooId) {
            $checkSql = 'SELECT fk_object FROM '.MAIN_DB_PREFIX.'societe_extrafields WHERE fk_object = :dol_id';
            $checkRes = $this->dbQueryParams($checkSql, ['dol_id' => ['value' => $dolId, 'type' => 'int']], 'Erreur lecture mapping tiers');

            if ($checkRes && $this->db->num_rows($checkRes) > 0) {
                $updateSql = 'UPDATE '.MAIN_DB_PREFIX.'societe_extrafields SET odoo_id = :odoo_id WHERE fk_object = :dol_id';
                $this->dbQueryParams($updateSql, [
                    'odoo_id' => ['value' => $odooId, 'type' => 'int'],
                    'dol_id' => ['value' => $dolId, 'type' => 'int'],
                ], 'Erreur update mapping tiers');
                return;
            }

            $insertSql = 'INSERT INTO '.MAIN_DB_PREFIX.'societe_extrafields (fk_object, odoo_id) VALUES (:dol_id, :odoo_id)';
            $this->dbQueryParams($insertSql, [
                'dol_id' => ['value' => $dolId, 'type' => 'int'],
                'odoo_id' => ['value' => $odooId, 'type' => 'int'],
            ], 'Erreur insert mapping tiers');
        });
    }
    ...
    public function testOdooConnectionDetailed()
    {
        ...
        // Step 2: Test HTTP
        try {
            $probe = $this->httpClient->request([
                'context' => 'odoo.connect.test',
                'url' => $this->odoo_url,
                'method' => 'HEAD',
                'connectTimeout' => 5,
                'timeout' => 10,
            ...
            $httpCode = (int) $probe['statusCode'];
        } catch (SyncHttpException $e) {
            $result['error'] = 'Serveur inaccessible: '.$e->getMessage();
    ...
    public function connectOdoo()
    {
        try {
            ...
            try {
                ...
                $this->lastError = 'Serveur Odoo inaccessible: '.$e->getMessage().' (vérifiez l\'URL '.$this->odoo_url.')';
                dol_syslog('SyncOdoo: '.$this->lastError, LOG_ERR);
                return false;
            }
    ...
    private function doliCallPublic($method, $endpoint, $data)
    {
        ...
        if ($method === 'POST' || $method === 'PUT') {
            ...
            if ($body === false) {
                throw new Exception('Encodage JSON Dolibarr impossible.');
            }
        }

        $response = $this->httpClient->request([
            'context' => 'dolibarr.api '.$method.' '.$endpoint,
            'url' => $url,
            'method' => $method,
            'headers' => [
        ...
        if ($httpCode < 200 || $httpCode >= 300) {
            throw new Exception('Dolibarr API error '.$httpCode.': '.$raw);
        }

        $decoded = json_decode($raw, true);
        if ($decoded === null && trim($raw) !== '' && trim($raw) !== 'null') {
            throw new Exception('Dolibarr API JSON invalide: '.$raw);
        }
    ...
    public function log($level, $direction, $entity_type, $entity_ref, $message)
    {
        $sql = "INSERT INTO ".MAIN_DB_PREFIX."syncodoo_log (datec, level, direction, entity_type, entity_ref, message)
                VALUES (NOW(), :level, :direction, :entity_type, :entity_ref, :message)";
        $this->dbQueryParams($sql, [
            'level' => ['value' => $level, 'type' => 'string'],
            'direction' => ['value' => $direction, 'type' => 'string'],
            'entity_type' => ['value' => $entity_type, 'type' => 'string'],
            'entity_ref' => ['value' => $entity_ref, 'type' => 'string'],
            'message' => ['value' => $message, 'type' => 'string'],
        ], 'Erreur insertion log syncodoo');
    }
    ...
    private function updateDolibarrBankLineDetails($bankLineId, $label, $note)
    {
        ...
        $sql = "UPDATE ".MAIN_DB_PREFIX."bank
                ...
                WHERE rowid = :rowid";
        $this->dbQueryParams($sql, [
            'label' => ['value' => substr((string) $label, 0, 255), 'type' => 'string'],
            'note' => ['value' => ($note !== '' ? $note : null), 'type' => ($note !== '' ? 'string' : 'null')],
            'rowid' => ['value' => $bankLineId, 'type' => 'int'],
        ], 'Erreur mise a jour ligne bancaire Dolibarr');
    }
    ...
    public function syncBankTransactions()
    {
        ...
        if ($direction === 'odoo_to_dolibarr' || $direction === 'both') {
            ...
            foreach ($transactions as $transaction) {
                ...
                if ($bankLineId > 0) {
                    $this->runInTransaction(function () use ($bankLineId, $transaction, $accountId, $journal) {
                ...
                $this->runInTransaction(function () use ($transaction, $account, $accountId, $journal) {
        ...
        if ($direction === 'dolibarr_to_odoo' || $direction === 'both') {
            ...
            foreach ($transactions as $transaction) {
                ...
                if ($odooTransactionId > 0) {
                    ...
                    $this->runInTransaction(function () use ($updated, $transaction, $odooTransactionId, $accountId, $journal, $current) {
                        if ($updated) {
                ...
                $this->runInTransaction(function () use ($newOdooTransactionId, $transaction, $accountId, $journal, $current) {
    ...
    private function getDolibarrCountryIdByCode($countryCode)
    {
        ...
        $sql = "SELECT rowid FROM ".MAIN_DB_PREFIX."c_country WHERE code = :country_code LIMIT 1";
        $resql = $this->dbQueryParams($sql, [
            'country_code' => ['value' => $countryCode, 'type' => 'string'],
        ], 'Erreur lecture pays Dolibarr');
        if ($resql) {
            $obj = $this->db->fetch_object($resql);
            if ($obj) {
    ...
    private function updateDolibarrThirdpartyCountry($dolThirdpartyId, $dolCountryId)
    {
        ...
        $sql = 'UPDATE '.MAIN_DB_PREFIX.'societe SET fk_pays = :country_id WHERE rowid = :thirdparty_id';
        $this->dbQueryParams($sql, [
            'country_id' => ['value' => $dolCountryId, 'type' => 'int'],
            'thirdparty_id' => ['value' => $dolThirdpartyId, 'type' => 'int'],
        ], 'Erreur mise a jour pays tiers Dolibarr');
    ...
    private function registerVatRateObservation($countryCode, $vatRate, $source, $invoiceRef)
    {
        ...
        $sql = "INSERT INTO ".MAIN_DB_PREFIX."syncodoo_vat_rate (country_code, vat_rate, confirmed, source, first_ref, last_ref, first_seen, last_seen)
                VALUES (:country_code, :vat_rate, 0, :source, :first_ref, :last_ref, NOW(), NOW())
        ...
        $this->dbQueryParams($sql, [
            'country_code' => ['value' => $countryCode, 'type' => 'string'],
            'vat_rate' => ['value' => (float) $safeRate, 'type' => 'float'],
            'source' => ['value' => (string) $source, 'type' => 'string'],
            'first_ref' => ['value' => (string) $invoiceRef, 'type' => 'string'],
            'last_ref' => ['value' => (string) $invoiceRef, 'type' => 'string'],
        ], 'Erreur enregistrement taux TVA');
    }
    ...
    public function confirmVatRateByRowId($rowId, $isExact, $correctRate = null)
    {
        ...
        $state = $isExact ? 1 : -1;
        $params = [
            'state' => ['value' => (int) $state, 'type' => 'int'],
            'row_id' => ['value' => (int) $rowId, 'type' => 'int'],
        ];

        $sql = "UPDATE ".MAIN_DB_PREFIX."syncodoo_vat_rate SET confirmed = :state, last_seen = NOW()";

        if (!$isExact && $correctRate !== null && $correctRate !== '' && is_numeric($correctRate)) {
            $params['correct_rate'] = ['value' => (float) $correctRate, 'type' => 'float'];
            $sql .= ", correct_rate = :correct_rate";
        }

        $sql .= " WHERE rowid = :row_id";
        $this->dbQueryParams($sql, $params, 'Erreur confirmation taux TVA');
    }
